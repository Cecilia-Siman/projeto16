...
export async function ranking(req,res) {
    try {
        ...
        'SELECT users.id, name, count(urls.id) as "linksCount", sum("visitCount") as "visitCount" from users left join urls on users.id = urls."userId" group by urls."userId", users.id  order by "visitCount" desc limit 10;'
        );
